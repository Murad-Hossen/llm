...
# Paths
# ---------------------------------------------------------------------------
SCRIPT_DIR   = Path(__file__).parent                        # llm_real_or_fake/
ARENA_DIR    = SCRIPT_DIR.parent / "Real_Or_Fake"           # competition repo root
...
EXPECTED_ROWS   = 1092   # Real_Or_Fake: 1092 test graphs
...
def validate_submission(arena_dir: Path) -> str:
    ...
    Real_Or_Fake requires: id,y_pred header with 1092 rows.
    """
...
def main():
    ...
    if not ARENA_DIR.exists():
        raise FileNotFoundError(
            f"Real_Or_Fake competition folder not found at: {ARENA_DIR}\n"
