...
def collate_fn(batch):
    ...
    return list(graphs), torch.tensor(labels)
...
class GNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = x.mean(dim=0, keepdim=True) # mean pooling
        x = self.pool(x)
        x = self.dropout(x)
        x = self.fc(x)
        return x
...
for epoch in range(100):
    ...
    # Validate the model
    model.eval()
    val_loader = DataLoader(train_dataset, batch_size=32, shuffle=False, collate_fn=collate_fn)
    total_correct = 0
    total_labels = []
    total_preds = []
    with torch.no_grad():
